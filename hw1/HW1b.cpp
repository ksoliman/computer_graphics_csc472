...
// HW1b.cpp - HW1b class
//
// Raymund Rodriguez + Kareem Soliman
// Mar 6 2019
//
...
HW1b::paintGL()
{
	...
	int colorsSize = (int) m_colors.size();
	// Draw points from vector m_points
	for(int i = 0, j = 0; i < colorsSize; i++) {
		// Set color based from m_colors
		glColor3f(m_colors[i][0], m_colors[i][1], m_colors[i][2]);
		// Draw points of triangles
...
HW1b::divideTriangle(vec2 a, vec2 b, vec2 c, int count)
{
	...
	if(count == 0) {
		...
		divideTriangle(bc, ac, ab, newCount);
	}
}
