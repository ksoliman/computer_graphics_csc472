...
HW2b::resizeGL(int w, int h)
{
    ...
    // xmax and ymax of the projection
    float xmax, ymax;
    // Wide screen (w > h)
    if (ar > 1.0) {
        xmax = ar;
    ...
    m_projection.setToIdentity(); // Identity matrix
...
HW2b::paintGL()
{
    ...
    // Bind vertex buffer to GPU; enable buffer access
    glBindBuffer(GL_ARRAY_BUFFER, m_vertexBuffer);
    glEnableVertexAttribArray(ATTRIB_VERTEX); // ATTRIB_VERTEX found in HW.h
    glVertexAttribPointer(ATTRIB_VERTEX, 2, GL_FLOAT, false, 0, NULL);
    
    // Bind color buffer to GPU; enable buffer access
    glBindBuffer(GL_ARRAY_BUFFER, m_colorBuffer);
    glEnableVertexAttribArray(ATTRIB_COLOR); // ATTRIB_COLOR found in HW.h
...
HW2b::divideTriangle(vec2 a, vec2 b, vec2 c, int count)
{
    ...
    if(count == 0) {
        ...
        vec2 ac = vec2((a[0] + c[0]) / 2.0, (a[1] + b[1]) / 2.0);
        ...
        // Order of vertices are correspondent with the positions of a, b, c of
        // original/bigger triangle but upside down.
        divideTriangle(bc, ab, ac, newCount);
    }
}
