...
HW3b::resizeGL(int w, int h)
{
	...
	m_projection.perspective(45.0f, ar, 0.1, 10.0);
}
...
HW3b::paintGL()
{
	...
	switch(m_displayMode) {
		...
		case TEXTURED:
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indicesBuffer[0]); // Indices of triangles
		...
		case WIREFRAME:
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indicesBuffer[1]); // Indices of wireframe
			...
			switch(m_geometryMode) {
				...
				case HOLE:
					...
					vec.setZ((!((i > m_grid / 3) && (j > m_grid / 3) &&
											(i < m_grid * 2 / 3) && (j < m_grid * 2 / 3))) ? 0.5 : 0.0);
					break;
				case DIAGONALWALL:
					// PUT YOUR CODE HERE
					vec.setZ((((m_grid - i) - j < 3) && ((m_grid - i) - j > 0)) ? 0.3 : 0.0);
					break;
				case SIDEWALL:
					// PUT YOUR CODE HERE
					vec.setZ((i == 1) ? 0.5 : 0.0);
					break;
				case DIAGONALBLOCK:
					// PUT YOUR CODE HERE
					vec.setZ(((m_grid - i) - j < 3) ? 0.5 : 0.0);
					break;
				case MIDDLEBLOCK:
					// PUT YOUR CODE HERE
					vec.setZ(((i > m_grid / 3) && (j > m_grid / 3) &&
				...
				case CORNERBLOCK:
					// PUT YOUR CODE HERE
					vec.setZ(((i > m_grid * 3 / 4 && j > m_grid * 3 / 4)) ? 0.5 : 0.0);
					break;
				case HILL:
					// PUT YOUR CODE HERE
					vec.setZ((sin(M_PI * ((float) i / (float) m_grid)) +
			              sin(M_PI * ((float) j / (float) m_grid))) * 0.3);
					break;
				case HILLFOUR:
					// PUT YOUR CODE HERE
					vec.setZ((sin(M_PI * 2 * ((float) i / (float) m_grid)) +
			              sin(M_PI * 2 * ((float) j / (float) m_grid))) * 0.3);
					break;
			}
		}
	}
