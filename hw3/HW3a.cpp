...
HW3a::paintGL()
{
    ...
    // Enable buffer to be copied to the attribute vertex variable
    // Specify data format
    glBindBuffer             (GL_ARRAY_BUFFER, m_vertexBuffer);
    glEnableVertexAttribArray(ATTRIB_VERTEX);
    glVertexAttribPointer    (ATTRIB_VERTEX, 2, GL_FLOAT, false, 0, NULL);
    
    // Bind texture coord buffer to the GPU
    // Enable buffer to be copied to the attribute texture coordinate variable
    // Specify data format
...
HW3a::initVertexBuffer()
{
    ...
    // Divide triangle recursively into smaller subtriangles
    // Store vertex positions and colors
    ...
    // Enable vertex buffer to be accessed thru the attribute vertex variable
    // Specify data format
    glEnableVertexAttribArray (ATTRIB_VERTEX);
    glVertexAttribPointer     (ATTRIB_VERTEX, 2, GL_FLOAT, false, 0, 0);
    
    // Bind texture coord buffer to the GPU
    // Copy the texture coordinates from CPU to GPU
    glBindBuffer(GL_ARRAY_BUFFER, m_texBuffer);
    glBufferData(GL_ARRAY_BUFFER, m_numPoints*sizeof(vec2), &m_coords[0], GL_STATIC_DRAW);
    
    // Enable buffer to be copied to the attribute texture coord variable
    // Specify data format
    glEnableVertexAttribArray (ATTRIB_TEXCOORD);
    glVertexAttribPointer     (ATTRIB_TEXCOORD, 2, GL_FLOAT, false, 0, NULL);
...
HW3a::divideTriangle(vec2 a, vec2 b, vec2 c, int count)
{
    if (count == 0)
        ...
        vec2 ab, bc, ac;
        ab = vec2((a[0] + b[0])/2, (a[1] + b[1])/2);
        ac = vec2((c[0] + a[0])/2, (c[1] + a[1])/2);
        bc = vec2((b[0] + c[0])/2, (b[1] + c[1])/2);
        
        // Recursively divide triangle into 4 smaller triangles
        int newCount = count - 1;
        divideTriangle(a,  ab, ac, newCount);
        divideTriangle(b,  ab, bc, newCount);
        divideTriangle(c,  ac, bc, newCount);
        divideTriangle(ab, bc, ac, newCount);
    }
}
